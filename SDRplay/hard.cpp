...
int mirisdr_set_hard(sdrplay_dev_t *p)
{
	int streaming = 0;
	...
	if (p->async_status == async_status_t::MIRISDR_ASYNC_RUNNING)
	{
		streaming = 1;

		if ((Sdrplay_stop_async(p) < 0) || (Sdrplay_adc_stop(p) < 0)) {
			return -1;
		}
	}
	...
	{
		if (p->rate <= 6048000) {
			p->format = format_t::MIRISDR_FORMAT_252_S16;
		} else if (p->rate <= 8064000) {
			p->format = format_t::MIRISDR_FORMAT_336_S16;
		} else if (p->rate <= 9216000) {
			p->format = format_t::MIRISDR_FORMAT_384_S16;
		} else {
			p->format = format_t::MIRISDR_FORMAT_504_S16;
		}
	}
	...
	fprintf( stderr, "rate: %u, vco: %lu (%lu), n: %lu, fraction: %lu\n",
			p->rate, (long uint32_t)vco, (long uint32_t)(i / 2) - 1,
			(long uint32_t)n, (long uint32_t)fract);
#endif
	...
	/* restart stream */
	if ((streaming) && (Sdrplay_start_async(p) < 0)) {
		return -1;
	}
...
int Sdrplay_set_sample_rate(sdrplay_dev_t *p, uint32_t rate)
{
	p->rate = rate;
...
uint32_t Sdrplay_get_sample_rate(sdrplay_dev_t *p)
{
	return p->rate;
}

int Sdrplay_set_sample_format(sdrplay_dev_t *p, format_t tt)
{
	if (p == NULL)
		return -1;
...
format_t Sdrplay_get_sample_format(sdrplay_dev_t *p)
{
	if (p->auto_format)
		p->format = MIRISDR_FORMAT_AUTO_ON;
